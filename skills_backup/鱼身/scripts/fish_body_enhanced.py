...
def main():
    ...
    out=f"/sandbox/workspace/outputs/fish_body_enhanced_{datetime.now().strftime('%Y%m%d_%H%M')}.json"
    try:
    ...
    try:
        ...
        md_file=f"/sandbox/workspace/outputs/鱼身报告_{now.strftime('%Y%m%d_%H%M')}.md"
        ...
        upload_cmd=f"python3 /sandbox/workspace/skills/ima-knowledge/scripts/upload_file.py --file-path {md_file} --knowledge-base-id {kb_id} --folder-id {folder_id} --rename '{rename}' 2>/dev/null"
