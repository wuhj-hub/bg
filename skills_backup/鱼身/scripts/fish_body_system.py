...
class StockPool:
    ...
    def generate_ashare_pool() -> list[str]:
        """生成A股全量代码池（沪市主板+深市主板，不含科创板/创业板/北交所）"""
        pool = []
        # 沪市主板 sh600000-sh605999，跳过sh688科创板
        for i in range(600000, 606000):
            code = f"sh{i}"
            if not (code.startswith('sh688') or code.startswith('sh689')):
                pool.append(code)
        # 深市主板 sz000001-sz003999，跳过sz300/sz301创业板
        for i in range(1, 4000):
            code = f"sz{i:06d}"
            if not (code.startswith('sz30') and code[4:7] in ['000','001']):
                pool.append(code)
        return pool
