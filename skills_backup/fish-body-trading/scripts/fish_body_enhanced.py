    ...
    if dif>0 and dea>0 and dif>=dea:
        ...
        if fs>=55:
            signals.append({'code':code,'name':name,'pattern':'MACD空中加油',
                ...
                'target':f"{close*1.15:.2f}" if close else "--",'mode':1})
    ...
    if ma20 and ma60 and ma20>ma60:
        ...
        if fs>=55:
            signals.append({'code':code,'name':name,'pattern':'均线回踩支撑',
                ...
                'target':f"{close*1.12:.2f}" if close else "--",'mode':2})
    ...
    if ma20 and ma60 and ma20>ma60:
        ...
        if fs>=55:
            signals.append({'code':code,'name':name,'pattern':'箱体突破',
                ...
                'target':f"{close*1.15:.2f}" if close else "--",'mode':3})
...
def main():
    ...
    if args.pool=='core': pool=core
        ...
        pool=[]
        for i in range(600000,606000): pool.append(f"sh{i}")
        for i in range(1,4000): pool.append(f"sz{i:06d}")
    else:
        try:
