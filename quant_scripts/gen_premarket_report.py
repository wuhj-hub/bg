...
ENV_SWITCH = {
    # 值 = (长期口径日均超额%, 短期口径日均超额%, 长期显著性, 短期显著性)
    # 显著性: 1=5%显著正 / .5=10%显著正 / 0=不显著 / -.5=10%显著负 / -1=5%显著负
    # 数据: 3,120只全历史(1991~2026) 按日聚合 + Newey-West t(lag=20)；已剔除|20日收益|>300%及单日|涨跌|>60% 的数据异常
    "一统天下·乖离低买(跌破MA5>7%)": {"熊市": (9.98, 4.43, 1, 1), "震荡市": (5.71, 8.13, 1, 1), "牛市": (7.64, 11.53, 1, 1)},
    "S1 超跌(RSI<25)": {"熊市": (2.46, 0.66, 1, 0), "震荡市": (0.64, 1.08, 0, 0), "牛市": (-0.18, 2.23, 0, 1)},
    "RSV启动(RSV均<20拐头↑)": {"熊市": (2.13, 0.22, 1, 0), "震荡市": (1.3, 1.75, 1, 1), "牛市": (1.44, 2.38, 0, 1)},
    "一统天下·日周双共振": {"熊市": (1.92, -0.3, 1, 0), "震荡市": (0.16, 1.64, 0, 1), "牛市": (-0.23, 1.55, 0, 0)},
    "猛兽·RS_D低吸": {"熊市": (1.3, 0.57, 1, 0.5), "震荡市": (0.15, 0.13, 0, 0), "牛市": (0.14, 0.84, 0, 0.5)},
    "猛兽·Setup≥60(领先股档)": {"熊市": (-1.54, 1.48, -1, 0), "震荡市": (0.06, 1.2, 0, 0), "牛市": (4.2, 1.6, 1, 0)},
    "武威G1·双阴(※月信号少)": {"熊市": (-0.42, 0.71, 0, 0), "震荡市": (2.19, 2.94, 0, 0), "牛市": (0.9, -1.06, 0, 0)},
    "双弦·低价共振(≤10元)": {"熊市": (-0.44, 0.0, 0, 0), "震荡市": (0.35, -0.12, 0, 0), "牛市": (0.09, 0.23, 0, 0)},
    "2B买入(20日新低收回2%)": {"熊市": (0.35, -0.48, 0, -1), "震荡市": (-0.08, -0.19, 0, 0), "牛市": (-0.8, 0.18, -1, 0)},
    "猛兽·Setup≥55": {"熊市": (-2.35, -0.67, -1, 0), "震荡市": (-0.58, -0.71, 0, -0.5), "牛市": (0.19, -0.53, 0, 0)},
    "一统天下·日线建仓区": {"熊市": (-0.09, -0.64, 0, -1), "震荡市": (-0.73, -0.37, -1, 0), "牛市": (-0.75, -0.59, -1, 0)},
    "猛兽·Setup≥50(初选档)": {"熊市": (-2.37, -1.64, -1, -1), "震荡市": (-1.54, -1.9, -1, -1), "牛市": (-1.74, -1.92, -1, -1)},
    "猛兽·伏击线低吸": {"熊市": (-2.42, -1.78, -1, -1), "震荡市": (-1.28, -1.7, -1, -1), "牛市": (-1.49, -1.61, -1, -1)},
    "鱼身·均线回踩支撑": {"熊市": (-2.79, -1.49, -1, -1), "震荡市": (-1.06, -1.82, -1, -1), "牛市": (-1.4, -1.75, -1, -1)},
    "鱼身·箱体突破": {"熊市": (-3.28, -2.31, -1, -1), "震荡市": (-2.14, -2.66, -1, -1), "牛市": (-2.17, -2.45, -1, -1)},
    "123买入(突破次高+回踩)": {"熊市": (-2.23, -1.88, -1, -1), "震荡市": (-1.81, -2.2, -1, -1), "牛市": (-1.85, -1.72, -1, -1)},
}
ENV_BUCKET = {"强势向上": "牛市", "向上": "牛市", "纠缠": "震荡市", "向下": "熊市", "弱势向下": "熊市"}
...
def render_env_switch():
    ...
    L = ["\n### ②.7 🧭 环境切换决策表（今天用哪套 · 关掉哪套）\n",
         ...
         "超额=日均（信号组合20日收益 − 同日全样本20日收益）。"
         "**数据：3,120只沪深主板全历史(1991~2026) + 按日聚合 + Newey-West t(lag=20)**；"
         "✅主用 = 两口径均值≥2% 且 双双5%显著。⚠️数值为「日均超额」，受右尾影响偏大，中位约为其 1/3。\n"]
    rows = []
    for nm, m in ENV_SWITCH.items():
        ...
        if lv > 0 and sv > 0:
            tag = ("✅ 主用" if (avg >= 2.0 and sig_pos >= 2)
                   else ("🟡 可用" if sig_pos >= 1 else "➖ 中性（正·不显著）"))
        elif lv < 0 and sv < 0:
            tag = "⛔ 关闭" if sig_neg >= 1 else "➖ 中性（弱负）"
        else:
            tag = "➖ 中性"
        rows.append((tag, nm, lv, sv, avg))
    _o = lambda t: 3 if t.startswith("✅") else (2 if t.startswith("🟡") else (0 if t.startswith("⛔") else 1))
    rows.sort(key=lambda r: (-_o(r[0]), -r[4]))
    act = [r for r in rows if r[0].startswith("✅") or r[0].startswith("🟡")]
    neu = [r for r in rows if r[0].startswith("➖")]
    off = [r for r in rows if r[0].startswith("⛔")]
    ...
    main = [short(r[1]) for r in act if r[0].startswith("✅")]
    ok = [short(r[1]) for r in act if r[0].startswith("🟡")]
    L.append("")
    ...
    L.append("- 说明：口径来自 `rsv_strength.py`/`scan_123_2b.py`/`wuwei_scan_month.py`/"
             "`fish_body_enhanced.py`/`beast_screener.py` 原始逻辑）。"
             "⚠️ 20年检验：**一统·乖离低买 6/6 环境全部5%显著正**（最强）｜RSV启动 4/6｜S1超跌 2/6｜"
