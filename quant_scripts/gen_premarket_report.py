...
ENV_SWITCH = {
    # 信号: {"环境": (长期力量口径超额%, 短期力量口径超额%)}
    "S1 超跌(RSI<25)":           {"熊": (7.99, 1.99),   "震荡": (3.47, 7.48),   "牛": (1.47, 8.41)},
    "RSV启动(RSV均<20拐头↑)":     {"熊": (2.51, 0.29),   "震荡": (0.50, 0.94),   "牛": (0.17, 1.97)},
    "猛兽·Setup≥60(领先股档)":    {"熊": (-0.93, -0.76), "震荡": (1.12, 1.35),   "牛": (1.59, 0.95)},
    "RSV半启动(20-40+涨停)":      {"熊": (2.33, -1.89),  "震荡": (-1.96, -0.41), "牛": (-1.64, 0.71)},
    "武威G1·双阴(阳2阴2缩量)":    {"熊": (0.44, 0.00),   "震荡": (-2.14, 0.47),  "牛": (2.14, 0.13)},
    "双弦·低价共振(≤10元)":       {"熊": (-0.46, 0.00),  "震荡": (0.60, 1.00),   "牛": (0.00, -1.12)},
    "猛兽·RS_D低吸":             {"熊": (0.01, -0.45),  "震荡": (-0.26, -0.01), "牛": (0.15, 0.27)},
    "猛兽·伏击线低吸":            {"熊": (-0.49, -0.67), "震荡": (-0.53, 0.03),  "牛": (-0.10, -0.61)},
    "2B买入(20日新低收回2%)":     {"熊": (0.11, -0.56),  "震荡": (-0.38, 0.06),  "牛": (0.13, 0.21)},
    "鱼身·均线回踩支撑":          {"熊": (-1.97, -1.27), "震荡": (-0.63, -0.45), "牛": (-0.80, -1.18)},
    "鱼身·箱体突破":             {"熊": (-2.31, -1.20), "震荡": (-0.89, -1.13), "牛": (-1.10, -1.42)},
    "RSV周线破50":               {"熊": (-1.57, -1.34), "震荡": (-0.43, -1.10), "牛": (-0.72, -0.32)},
    "123买入(突破次高+回踩)":     {"熊": (-1.66, -1.01), "震荡": (-0.64, -0.51), "牛": (-0.50, -0.96)},
}
...
def render_env_switch():
    ...
    L = ["\n### ②.7 🧭 环境切换决策表（今天用哪套 · 关掉哪套）\n",
         ...
         "超额=信号事件20日收益−**同期全样本20日收益**（361万观测）。"
         "猛兽Setup=十维中八维量价口径（缺净利润断层10分+基本面5分，故绝对分偏低、阈值为同口径）。\n"]
    rows = []
