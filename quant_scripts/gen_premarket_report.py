...
ENV_SWITCH = {
    # 值 = {"环境": (长期口径日均超额%, 短期口径日均超额%, 长期显著性, 短期显著性)}
    # 显著性: 1=5%显著为正 / .5=10%显著为正 / 0=不显著 / -.5=10%显著为负 / -1=5%显著为负
    # 数据: 3,120只沪深主板全历史(1991~2026)，按日聚合 + Newey-West t 检验(lag=20)
    "S1 超跌(RSI<25)":          {"熊": (5.07, 1.67, 1, 1),    "震荡": (2.19, 2.51, 1, .5),  "牛": (-0.04, 5.47, 0, 1)},
    "RSV启动(RSV均<20拐头↑)":    {"熊": (2.05, 7.69, 1, 0),    "震荡": (1.76, 2.62, 1, 1),   "牛": (11.75, 6.97, .5, .5)},
    "猛兽·RS_D低吸":             {"熊": (1.66, 0.50, 1, 0),    "震荡": (0.59, 0.03, 0, 0),   "牛": (0.22, 2.07, 0, .5)},
    "猛兽·Setup≥60(领先股档)":    {"熊": (-0.91, 1.24, 0, 0),  "震荡": (-0.16, 1.22, 0, 0),  "牛": (5.22, 3.24, 1, 0)},
    "武威G1·双阴(※样本少)":      {"熊": (2.87, 3.28, 0, 0),    "震荡": (5.57, 4.30, 0, 0),   "牛": (0.28, 1.45, 0, 0)},
    "双弦·低价共振(≤10元)":      {"熊": (-0.49, 0.00, 0, 0),   "震荡": (0.05, -0.57, 0, -1), "牛": (-0.04, 0.40, 0, 0)},
    "2B买入(20日新低收回2%)":     {"熊": (0.51, 6.45, 0, 0),    "震荡": (6.69, -0.14, 0, 0),  "牛": (-1.21, 0.09, -1, 0)},
    "猛兽·伏击线低吸":            {"熊": (-3.03, -2.30, -1, -1), "震荡": (-1.90, -2.24, -1, -1), "牛": (-2.10, -2.41, -1, -1)},
    "鱼身·均线回踩支撑":          {"熊": (-3.35, -2.03, -1, -1), "震荡": (-1.50, -2.33, -1, -1), "牛": (-2.03, -2.33, -1, -1)},
    "鱼身·箱体突破":              {"熊": (-3.79, -2.74, -1, -1), "震荡": (-2.56, -3.14, -1, -1), "牛": (-2.78, -3.08, -1, -1)},
    "123买入(突破次高+回踩)":      {"熊": (-2.71, -2.31, -1, -1), "震荡": (-2.26, -2.62, -1, -1), "牛": (-2.30, -2.26, -1, -1)},
}
...
def render_env_switch():
    ...
    L = ["\n### ②.7 🧭 环境切换决策表（今天用哪套 · 关掉哪套）\n",
         ...
         "**数据：3,120只沪深主板全历史(1991~2026) + Newey-West t 检验(lag=20)**；"
         "✅主用 = 两口径均值≥2% 且 双双5%显著。\n"]
    rows = []
    ...
    L.append("- 说明：口径来自 `rsv_strength.py`/`scan_123_2b.py`/`wuwei_scan_month.py`/"
             ...
             "⚠️ 只有 **S1超跌 / RSV启动 / 猛兽RS_D** 通过 20 年显著性检验；"
             "鱼身三形态、猛兽伏击线、123买入在**全部 6 个环境下显著为负**（t≈-6~-11）。"
             "详见技能《板块个股入牛时点》第九节。\n")
    return "\n".join(L)
